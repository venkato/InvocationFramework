...
public class IdeaBuilderAddGroovyRuntime {
    ...
    public static void f1() throws Exception {
        try {
            ...
            if(IdeaBuildRunnerSettings.jrrIdeaForceUseStd){
                LauncherOriginal.main(IdeaBuildRunnerSettings.argsP);
            }else {
                doJobImpl();
            }
        }catch (Throwable e){
    ...
    public static void doRedirect() throws Exception {
        File jrrlibpath = new File(System.getProperty("user.home")+"/idea_build_jrr_log.txt");
        //FileOutputStream fous = new FileOutputStream(jrrlibpath);
        PrintStream printStream1= new PrintStream(jrrlibpath);
...
//        jrrpath = jrrPathDefault.getAbsolutePath();
    ...
    public static File detectJrrPath() throws Exception {
        ...
        File jrrlibpath = new File(System.getProperty("user.home")+"/jrr/configs/jrrlibpath.txt");
        boolean jrrlibpathE = jrrlibpath.exists();
        log.info("jrrlibpath exit : "+jrrlibpathE+" "+jrrlibpath);
        if(jrrlibpathE){
            FileInputStream fis = new FileInputStream(jrrlibpath);
            try{
                byte[] buff= new byte[10000];
                int read = fis.read(buff);
                if(read<2){
                    throw new IOException("Failed read : "+jrrlibpath.getAbsolutePath());
                }
