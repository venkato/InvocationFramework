 ...
 */
public class RemoteSnapshotMetadataJrr
    extends MavenSnapshotMetadata
{
    ...
    public RemoteSnapshotMetadataJrr(Artifact artifact, boolean legacyFormat, RepositorySystemSession session )
    {
        super( createRepositoryMetadata( artifact, legacyFormat ), null, legacyFormat );
        this.session = session;
    }

    private RemoteSnapshotMetadataJrr(Metadata metadata, File file, boolean legacyFormat, RepositorySystemSession session )
    {
    ...
    {
        return new RemoteSnapshotMetadataJrr( metadata, file, legacyFormat ,session);
    }
    ...
    protected void merge( Metadata recessive )
    {
        ...
        String lastUpdated;
        boolean createSnapshots = true;
        MavenSettingsJrr objs = (MavenSettingsJrr) session.getConfigProperties().get(MavenSettingsJrr.mavenSettingsJrrS);
        if(objs!=null){
            createSnapshots = objs.createSnapShotUpload;
        }
        ...
        if ( metadata.getVersioning() == null   )
        {
            ...
            if(createSnapshots) {
                ...
                lastUpdated = utcDateFormatter.format(new Date()).replace(".", "");
                Versioning versioning = new Versioning();
        ...
        for ( Artifact artifact : artifacts )
        {
            ...
            if ( version.endsWith( SNAPSHOT ) && createSnapshots )
            {
