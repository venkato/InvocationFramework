...
public class CronExpression implements Serializable, Cloneable {
	...
	protected int storeExpressionVals(final int pos, final String s, final int type) throws ParseException {
		...
		if (c == '?') {
			...
			if (type == DAY_OF_WEEK && !lastdayOfMonth) {
				final int val = ((Integer) daysOfMonth.last()).intValue();
				if (val == NO_SPEC_INT) {
					throw new ParseException("'?' can only be specfied for Day-of-Month -OR- Day-of-Week.", i);
				}
			}
	...
	protected TreeSet<Integer> getSet(final int type) {
		switch (type) {
		case SECOND:
			return seconds;
		case MINUTE:
	...
	protected int getMonthNumber(final String s) {
		final Integer integer = (Integer) monthMap.get(s);
	...
	protected int getDayOfWeekNumber(final String s) {
		final Integer integer = (Integer) dayMap.get(s);
		...
		while (!gotOne) {
			...
			if (dayOfMSpec && !dayOfWSpec) { // get day by day of month rule
				...
				if (lastdayOfWeek) { // are we looking for the last XXX day of
					// the month?
					final int dow = ((Integer) daysOfWeek.first()).intValue(); // desired
					// d-o-w
	...
	public Object clone() {
		...
		} catch (final ParseException ex) { // never happens since the source is
			// valid...
