...
public class RemoteService {
	...
	public static RemoteService defaultRemoteService = new RemoteService();
	...
	public void handleSocketNewThread(final TcpSession tcpSession) {
...
//			.getBytes();
	...
	private static ByteArrayOutputStream checkNext(ByteArrayOutputStream out,
		...
		String s = new String(out.toByteArray(), TimmosonSettings.defaultEncoding);
		// log.info(s);
	...
	public byte[][] findBetween(String begin, String end,
			InputStream in, ByteArrayOutputStream out) throws Exception {

		int k = waitWord(in, out, end);

		byte[] bb = out.toByteArray();
		String dsfd = new String(bb, TimmosonSettings.defaultEncoding);
		log.debug(dsfd);
		...
		log.debug(beginI);
		log.debug(endI);
		log.debug(new String(bb, beginI, diff + 1, TimmosonSettings.defaultEncoding));
